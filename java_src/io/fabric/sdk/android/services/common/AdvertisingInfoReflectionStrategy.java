...
class AdvertisingInfoReflectionStrategy implements AdvertisingInfoStrategy {
    ...
    public boolean isGooglePlayServiceAvailable(Context context2) {
        try {
            if (((Integer) Class.forName(CLASS_NAME_GOOGLE_PLAY_SERVICES_UTILS).getMethod(METHOD_NAME_IS_GOOGLE_PLAY_SERVICES_AVAILABLE, Context.class).invoke(null, context2)).intValue() == 0) {
                return true;
            }
            return false;
