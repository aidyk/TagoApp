...
public final class ThreadLocalElement<T> implements ThreadContextElement<T> {
    @NotNull
    private final CoroutineContext.Key<?> key = new ThreadLocalKey(this.threadLocal);
    private final ThreadLocal<T> threadLocal;
    private final T value;

    public ThreadLocalElement(T t, @NotNull ThreadLocal<T> threadLocal2) {
    ...
    public CoroutineContext minusKey(@NotNull CoroutineContext.Key<?> key2) {
        ...
        return Intrinsics.areEqual(getKey(), key2) ? EmptyCoroutineContext.INSTANCE : this;
    }
    ...
    public <E extends CoroutineContext.Element> E get(@NotNull CoroutineContext.Key<E> key2) {
        Intrinsics.checkParameterIsNotNull(key2, "key");
        if (Intrinsics.areEqual(getKey(), key2)) {
            return this;
        }
        return null;
    }
    ...
    public String toString() {
        return "ThreadLocal(value=" + ((Object) this.value) + ", threadLocal = " + this.threadLocal + ')';
    }
}
