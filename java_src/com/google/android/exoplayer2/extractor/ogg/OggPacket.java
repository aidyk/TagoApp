...
public final class OggPacket {
        ...
        while (!this.populated) {
            ...
            if (calculatePacketSize > 0) {
                if (this.packetArray.capacity() < this.packetArray.limit() + calculatePacketSize) {
                    this.packetArray.data = Arrays.copyOf(this.packetArray.data, this.packetArray.limit() + calculatePacketSize);
                }
                extractorInput.readFully(this.packetArray.data, this.packetArray.limit(), calculatePacketSize);
                this.packetArray.setLimit(this.packetArray.limit() + calculatePacketSize);
    ...
    public void trimPayload() {
        if (this.packetArray.data.length != 65025) {
            this.packetArray.data = Arrays.copyOf(this.packetArray.data, Math.max((int) OggPageHeader.MAX_PAGE_PAYLOAD, this.packetArray.limit()));
        }
    }
